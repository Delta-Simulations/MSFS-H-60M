...
double cargoOffsetAltitude = 0.0;
...
void setUpCargo(double latitude, double longitude, double altitude, double heading)
{
    ...
    Init.Altitude = altitude + cargoOffsetAltitude;      // Altitude above sea level
    ...
    hr = SimConnect_AICreateSimulatedObject(hSimConnect, "H60 ISU Cargo", Init, REQUEST_CARGO_POSITION);
    if (FAILED(hr))
    {
...
void CALLBACK MyDispatchProcSO(SIMCONNECT_RECV* pData, DWORD cbData, void* pContext)
{
    ...
    case SIMCONNECT_RECV_ID_EVENT:
    {
        ...
        case EVENT_Z:
            ...
            hr = SimConnect_RequestDataOnSimObject(hSimConnect, REQUEST_CARGO_POSITION, SIMCONNECT_OBJECT_ID_USER, DEFINITION_LVAR_CARGO_OFFSET, SIMCONNECT_PERIOD_ONCE, 0, 0, 0, 0);
            if (FAILED(hr)) {
                printf("Failed to request cargo offset. HRESULT: %d\n", hr);
            }
    ...
    case SIMCONNECT_RECV_ID_SIMOBJECT_DATA:
    {
        ...
        if (pObjData->dwRequestID == REQUEST_USER_POSITION)
        {
            ...
            double altitude = pUserPos->Altitude + cargoOffsetAltitude;
        ...
        else if (pObjData->dwRequestID == REQUEST_CARGO_POSITION)
        {
            double cargoOffset = *(double*)&pObjData->dwData;
            printf("Cargo Offset: %f\n", cargoOffset);

            cargoOffsetAltitude = cargoOffset;
        }
    ...
    case SIMCONNECT_RECV_ID_SIMOBJECT_DATA_BYTYPE:
    {
        ...
        if (pObjData->dwRequestID == REQUEST_USER_POSITION)
        {
            ...
            printf("User Position (By Type) - Lat: %f, Lon: %f, Alt: %f, Heading: %f\n", latitude, longitude, altitude, heading);

            // Add a parked "Spawning_Cargo" at the user's location
            setUpCargo(latitude, longitude + cargoOffsetAltitude, altitude , heading);
        }
        break;
    }
...
void testCargoSpawn()
{
    ...
    if (SUCCEEDED(SimConnect_Open(&hSimConnect, "Cargo Debug Spawn", NULL, 0, 0, 0)))
    {
        ...
        // Set up a def for cargo position offset
        hr = SimConnect_AddToDataDefinition(hSimConnect, DEFINITION_LVAR_CARGO_OFFSET, "L:H60_Cargo_Offset_Alt", "feet", SIMCONNECT_DATATYPE_FLOAT64);
