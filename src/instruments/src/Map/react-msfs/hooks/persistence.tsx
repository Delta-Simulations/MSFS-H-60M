...
import { useSimVar } from '@instruments/Hooks/simVars';
...
export const usePersistentProperty = (propertyName: string): [string, (string) => void] => {
    const [propertyValue, rawPropertySetter] = useState(() => FsDataStore.get(propertyName));

    const propertySetter = (value: string) => {
        FsDataStore.set(propertyName, value);
...
export const usePersistentPropertyWithDefault = (propertyName: string, defaultValue: string): [string, (string) => void] => {
    const [propertyValue, rawPropertySetter] = useState(() => FsDataStore.get(propertyName, defaultValue));
